...
# restore
if args.restore:
    restore_path = os.path.join(args.restore, 'weights/params.ckpt')
    print(f"Restoring model from path {restore_path}")
...
for epoch in range(params.epochs):
    ...
    if test_metric > best_test_metric:
        ...
        save_path = os.path.join(params.exp_dir, 'weights/params.ckpt')
        logging.info(f"Saving best model at path {save_path}")
        saver.save(sess, save_path)
    # if args.save_last:
    #     save_path = os.path.join(params.exp_dir, 'weights/params.ckpt')
    #     saver.save(sess, save_path)
