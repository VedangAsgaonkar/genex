...
def sample(q_val, available, exist, eps=None):
    ...
    initial = (~torch.eq(available, exist)).long()
    ...
    max_q_val, action = q_val[:,:-1].max(dim=1)
    # print(torch.argsort(torch.sum(q_val[:,:-1], dim=0),descending=True))
...
def test(step_runner, env, args, iter=0):
    ...
    while offset < n_data:
        ...
        ct += 1
        if ct==T+1:
            actions = n_features*torch.ones_like(actions)
            ct = 0
...
def learn(step_runner, args, env, valenv=None, nsteps=5,
    ...
    if 'test' not in args.message:
      ...
      print("valinput shape:", valinput.shape)
    ...
    #   print("PRETRAIN TIME", pretrain_time)
      step_runner.load(os.path.join(args.save_path, "pretrained_best.model"))
      env.reset()
      ###################
      # Pretraining end
      ###################
      print("Finished pretrain", flush = True)
...
def test_and_record(step_runner, args, env, valenv=None, testenv=None):
    ...
    field = []
    result = {}
    for prefix, environ in [('ts', testenv)]:
...
def main(args):
    ...
    comb_data, cost = data_load(data_type=args.data_type, csv_filename = "./data/mnist/Training.csv",
              random_seed=args.random_seed, cost_from_file=args.cost_from_file)
    traindata, valdata, testdata = comb_data
    if cost is not None:
        r_cost = cost
    ...
    dfsnet = DFSNet(
        ...
        policy=DuelingNet(lstm_size + embedded_dim, policy_hidden_sizes, shared_dim,
            traindata.n_actions))
    print(sum(p.numel() for p in dfsnet.parameters()))
    input()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--gpu', help='gpu index', type=int, default=2
    )
