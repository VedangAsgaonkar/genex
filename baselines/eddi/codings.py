...
def fc_categorical_decoder(z, obs_dim, n_categories, activation=tf.nn.sigmoid):
    x = layers.fully_connected(z, 50, scope='fc-01')
    x = layers.fully_connected(x, 100, scope='fc-02')
    out1 = layers.fully_connected(x, obs_dim-1, activation_fn=tf.nn.sigmoid, scope='fc-final1')
    out2 = layers.fully_connected(x, n_categories, activation_fn=tf.nn.softmax, scope='fc-final2')
