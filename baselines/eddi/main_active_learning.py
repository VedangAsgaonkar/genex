# python main_active_learning.py --epochs 200 --eval nllh --latent_dim 32 --p 0.99 --data_dir ./data/disease_pred --output_dir ./model
'''
...
### load data
Data_train = pd.read_csv(UCI + '/Training1.csv')
# Data = Data.loc[:, ~Data.columns.str.contains('^Unnamed')]
# Data.prognosis = pd.Categorical(Data.prognosis)
# Data['prognosis'] = Data.prognosis.cat.codes
# print(Data)
Data_train = Data_train.values
Data_test = pd.read_csv(UCI + '/Validation1.csv')
Data_test = Data_test.values
test_offset = Data_train.shape[0]
print(Data_train.shape, Data_test.shape)

Data = np.vstack((Data_test, Data_train))

### data preprocess
max_Data = 1  #
min_Data = 0  #
Data_std = (Data[:,:-1] - Data[:,:-1].min(axis=0)) / (Data[:,:-1].max(axis=0) - Data[:,:-1].min(axis=0))
Data_std[np.isnan(Data_std)] = 0
Data[:,:-1] = Data_std * (max_Data - min_Data) + min_Data
Mask = np.ones(Data.shape) # this UCI data is fully observed
Data_train, Data_test, mask_train, mask_test = Data[:test_offset], Data[test_offset:], Mask[:test_offset], Mask[test_offset:]
