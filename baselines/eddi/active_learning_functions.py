...
def p_vae_active_learning(Data_train,mask_train,Data_test,mask_test,epochs,latent_dim,batch_size,p,K,M,eval,Repeat,estimation_method=0):
    ...
    for r in range(Repeat):
        ...
        n_categories = len(np.unique(np.reshape(Data_test[:,-1], (-1,))))
        ...
        if r == 0:
            ...
            # outputs
            # output_CHAI = np.zeros(
            #     (Repeat, n_test, OBS_DIM - 1 + n_categories))
        ...
        for strategy in range(2,3):
            ...
            if strategy == 0:### random strategy
                ...
                mask = np.ones((n_test, OBS_DIM))
                mask[:,-1] = 0
                print("---------------FULL---------------")
                negative_predictive_llh, uncertainty, output = vae.predictive_loss(
                    x, mask, eval,M, n_categories)
                print("----------------------------------")
                ...
                for t in range(OBS_DIM - 1): # t is a indicator of step
                    ...
                    im = completion(x, mask, M, vae, n_categories)
                    # im_CHAI[r, t, :, :, :] = im
                    ...
                    negative_predictive_llh, uncertainty,_ = vae.predictive_loss(
                        x, mask, eval,M)
                    mask2 = mask2 + io # this mask only stores missingess of unselected features, i.e., which features has been selected of each data
                    print("nllh = {:.4f}".format(np.mean(negative_predictive_llh)))
...
def train_p_vae(Data_train,mask_train, epochs, latent_dim,batch_size, p, K,iteration):
    ...
    n_categories = len(np.unique(np.reshape(Data_train[:,-1], (-1,))))
    n_train = Data_train.shape[0]
    list_train = np.arange(n_train)
    ####### construct
    kwargs = {
        'K': K,
        'obs_distrib': "Categorical",
        'latent_dim': latent_dim,
        'batch_size': batch_size,
        'encoder': PNP_fc_uci_encoder,
        'decoder': fc_categorical_decoder,
        'obs_dim': obs_dim,
        'load_model':0,
