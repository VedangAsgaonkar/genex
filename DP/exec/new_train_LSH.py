...
parser.add_argument('--warm_end_S', action='store_true', default=False)
...
if args.train_gen:
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(num_batches):
            ...
            # mask = torch.bernoulli(torch.ones(X.shape)*args.p)*present_mask
...
while epoch<300 and not args.no_greedy: # WARNING: hard coded epochs
    epoch_loss = 0
    epoch+=1
    ugf = 0
    transformer_encoder.train()
...
if args.warm_end_S:
    ...
    suffix = f'_ns{args.num_sets}_model_final__warm_end_end_to_end_{args.warm_end_end_to_end}__warm_end_loss_{args.warm_end_loss}'
    ...
    torch.save(classifier.state_dict(), os.path.join(save_folder, f'classifier_S_only__LSH_c{c}' + suffix))
    torch.save(set_embedding.state_dict(), os.path.join(save_folder, f'set_embedding_S_only_99_LSH_c{c}' + suffix))
    torch.save(transformer_encoder.state_dict(), os.path.join(save_folder, f'encoder_S_only_99_LSH_c{c}' + suffix))
    torch.save(decoder.state_dict(), os.path.join(save_folder, f'decoder_S_only_99_LSH_c{c}' + suffix))
    ...
    pickle.dump(acc.item(), open(os.path.join(save_metrics_folder, f"train_S_only_acc_LSH__c{c}__warm_end_end_to_end_{args.warm_end_end_to_end}__warm_end_loss_{args.warm_end_loss}__generate_during_test_{args.generate_during_test}.pkl"), "wb"))
...
if(args.test):
    ...
    with torch.no_grad():
        ...
        for i in range(num_batches):
            ...
            accuracies[-1] += torch.sum(Y==Y_hat).cpu().detach().item()
            sizes[-1] += Y.shape[0]
    ...
    with open(os.path.join(save_metrics_folder, f"random_subset__c_{c}__T_{args.warm_end_load_T}__warm_end_end_to_end_{args.warm_end_end_to_end}__warm_end_loss_{args.warm_end_loss}__generate_during_test_{args.generate_during_test}__generate_for_min_{args.generate_for_min}.pkl"), "wb") as f:
