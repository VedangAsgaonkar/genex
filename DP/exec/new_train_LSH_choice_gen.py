...
if args.train_gen:
    ...
    if args.data=='mnist':
        ...
        saved_num = 99
    set_embedding.load_state_dict(torch.load(os.path.join(save_folder, f'set_embedding_{saved_num}')))
    transformer_encoder.load_state_dict(torch.load(os.path.join(save_folder, f'encoder_{saved_num}')))
    decoder.load_state_dict(torch.load(os.path.join(save_folder, f'decoder_{saved_num}')))
...
if args.warm_end:
    if(args.warm_end_load_T >max_T):
        ...
        exit()
    classifier_path = f"classifier__LSH_c{c}_ns20_{2*args.warm_end_load_T + 2}_sizeofT{args.warm_end_load_T}"
    T = pickle.load(open(os.path.join(save_folder, f'T__LSH_c{c}_ns20_{2*args.warm_end_load_T+2}_sizeofT{args.warm_end_load_T}'), "rb"))
    small_T_size = 10
    T_small = pickle.load(open(os.path.join(save_folder, f'T__LSH_c{c}_ns20_{2*small_T_size+2}_sizeofT{small_T_size}'), "rb"))
    ...
    transformer_encoder_gen = copy.deepcopy(transformer_encoder).to(device)
    set_embedding_gen = copy.deepcopy(set_embedding).to(device)
    decoder_gen = copy.deepcopy(decoder).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(num_batches):
            ...
            mask_new = copy.deepcopy(mask_S)
            mask_new[:, T] = 1
            mask_SUT_small = copy.deepcopy(mask_S)
            mask_SUT_small[:,T_small] = 1
            optimizer.zero_grad()
            z, _, _ = VAE_sample(transformer_encoder_gen, set_embedding_gen(X, mask_SUT_small), args.latent_dim)
            X_g = decoder_gen(z)
            X_combined = X*mask_SUT_small + X_g*(1-mask_SUT_small)
    ...
    torch.save(classifier.state_dict(), os.path.join(save_folder, f'classifier__LSH_gen_c{c}' + suffix))
    torch.save(set_embedding.state_dict(), os.path.join(save_folder, f'set_embedding_99_LSH_gen_c{c}' + suffix))
    torch.save(transformer_encoder.state_dict(), os.path.join(save_folder, f'encoder_99_LSH_gen_c{c}' + suffix))
    torch.save(decoder.state_dict(), os.path.join(save_folder, f'decoder_99_LSH_gen_c{c}' + suffix))
...
if(args.test):
    ...
    transformer_encoder_sample.load_state_dict(torch.load(os.path.join(save_folder, f'encoder_99_LSH_gen_c{c}' + suffix), map_location=device))
    set_embedding_sample.load_state_dict(torch.load(os.path.join(save_folder, f'set_embedding_99_LSH_gen_c{c}' + suffix), map_location=device))
    decoder_sample.load_state_dict(torch.load(os.path.join(save_folder, f'decoder_99_LSH_gen_c{c}' + suffix), map_location=device))
    classifier_sample.load_state_dict(torch.load(os.path.join(save_folder, f'classifier__LSH_gen_c{c}' + suffix), map_location=device))
    ...
    with torch.no_grad():
        ...
        bucket_size = int(math.ceil(len(X_query)/10))
        print(bucket_size)
        ...
        for i in range(num_batches):
            ...
            correct_arr[i*bs//bucket_size] += torch.sum(Y==Y_hat).cpu().detach().item()
            total_arr[i*bs//bucket_size] += len(Y)
        print("STD",np.std(correct_arr/total_arr)/np.sqrt(160))

        num_batches = math.ceil(len(X_sample)/args.batch_size)
        bucket_size = int(math.ceil(len(X_sample)/10))
        print(bucket_size)
        for i in range(num_batches):
