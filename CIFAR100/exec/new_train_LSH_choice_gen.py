...
parser.add_argument('--choice_frac_S', default=0.03, type=float)
parser.add_argument('--test_three_way', action='store_true', default=False)
parser.add_argument('--train_three_way', action='store_true', default=False)
...
if args.train_gen:
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            mask_S = set_to_mask(getT2(s), 1024)
...
test_data_subset = torch.utils.data.Subset(val_dataset, val_indices[c])
testloader = torch.utils.data.DataLoader(test_data_subset, batch_size=bs, shuffle=False)
...
if args.warm_end:
    ...
    if c==0:
        ...
        e_small = T_small_len//2+4
        if T_small_len==80:
            e_small = 43
    T_small = pickle.load(open(os.path.join(save_folder, f'T__LSH_c{c}_ns{args.num_sets}_{e_small}_sizeofT{T_small_len}'), "rb"))
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            mask_small_TUS = copy.deepcopy(mask_S)
            mask_small_TUS[:, getT2(T_small)] = 1
            mask_S = mask_S.reshape((-1,32,32)).to(device)
            mask_small_TUS = mask_small_TUS.reshape((-1,32,32)).to(device)
            mask_new = mask_new.reshape((-1,32,32)).to(device)
            optimizer.zero_grad()
            inputs_g = decoder(encoder(inputs*mask_small_TUS[:,None,:,:]))
            inputs_comb = inputs*mask_small_TUS[:,None,:,:] + inputs_g*(1-mask_small_TUS)[:,None,:,:]
            outputs = classifier(inputs_comb*mask_new[:,None,:,:])               # Forward Propagation
            batch_loss = loss(outputs, targets)  # Loss
            batch_loss.backward()  # Backward Propagation
            optimizer.step() # Optimizer update
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                for batch_idx, (inputs, targets) in enumerate(testloader):
                    ...
                    inputs_g = decoder(encoder(inputs*mask_small_TUS[:,None,:,:]))
                    inputs_comb = inputs*mask_small_TUS[:,None,:,:] + inputs_g*(1-mask_small_TUS)[:,None,:,:]
                    outputs = classifier(inputs_comb*mask_new[:,None,:,:])
                    batch_loss = loss(outputs, targets)
    ...
    torch.save(classifier.state_dict(), os.path.join(save_folder, f'classifier_small_T__LSH_c{c}' + suffix))

    path_T = os.path.join(save_folder, f'T_small_T_LSH_c{c}' + suffix)
    pickle.dump(T, open(path_T, 'wb'))


    print("DATA SIZE", len(data_subset))
    print("FINAL ACC", acc.item())
    print("DATA SIZE*ACC", len(data_subset)*acc.item())

    pickle.dump(acc.item(), open(os.path.join(save_metrics_folder, f"train_small_T_acc_LSH__c{c}_{args.warm_end_load_T}__warm_end_end_to_end_{args.warm_end_end_to_end}__warm_end_loss_{args.warm_end_loss}__generate_during_test_{args.generate_during_test}__warm_end_fresh_start_{args.warm_end_fresh_start}.pkl"), "wb"))
...
if(args.test):
    ...
    if c==0:
        ...
        if args.warm_end_load_T==80:
            e = 43
    classifier_path = f"classifier__LSH_c{c}_ns{args.num_sets}_model_final__warm_end_end_to_end_{args.warm_end_end_to_end}__warm_end_loss_{args.warm_end_loss}__T_{args.warm_end_load_T}_warm_end_fresh_start_{args.warm_end_fresh_start}"
    T = pickle.load(open(os.path.join(save_folder, f'T__LSH_c{c}_ns{args.num_sets}_{e}_sizeofT{args.warm_end_load_T}'), "rb"))
    classifier.load_state_dict(torch.load(os.path.join(save_folder, classifier_path), map_location=device))

    classifier_gen = Wide_ResNet(28,10,0.3,100).to(device)
    classifier_gen_path = f'classifier_small_T__LSH_c{c}_ns{args.num_sets}_model_final__warm_end_end_to_end_{args.warm_end_end_to_end}__warm_end_loss_{args.warm_end_loss}__T_{args.warm_end_load_T}_warm_end_fresh_start_{args.warm_end_fresh_start}'
    classifier_gen.load_state_dict(torch.load(os.path.join(save_folder, classifier_gen_path), map_location=device))
    args.warm_end_load_T -= 5
    T = T[:args.warm_end_load_T]
    T_small_len = 30
    if c==0:
        e_small = T_small_len//5+2
        if T_small_len==70:
    ...
    random_indices = np.random.permutation(len(val_indices[c]))
    bucket_size = int(math.ceil(len(val_indices[c])/1))
    results = []
    for mc_i in range(10):
        random_test_data_subset = torch.utils.data.Subset(test_data_subset, random_indices[bucket_size*mc_i:bucket_size*(1+mc_i)])
        random_testloader = torch.utils.data.DataLoader(random_test_data_subset,batch_size=bs, shuffle=False)
        # print(random_indices.shape, bucket_size, mc_i, testmasks.shape)
        random_testmasks = testmasks[random_indices[bucket_size*mc_i:bucket_size*(1+mc_i)]]
        total = 0
        correct = 0
        Q_plus = 0
        Q_minus = 0
        ...
        with torch.no_grad():
            ...
            correct = 0
            total = 0
            for batch_idx, (inputs, targets) in enumerate(random_testloader):
                inputs, targets = inputs.to(device), targets.to(device)
                mask_S = random_testmasks[batch_idx*bs:(1+batch_idx)*bs]
                mask_new = copy.deepcopy(mask_S)
                mask_new[:, getT2(T_val)] = 1
                mask_small_TUS = copy.deepcopy(mask_S)
                mask_small_TUS[:, getT2(T_small)] = 1
                ...
                inputs_comb = inputs*mask_small_TUS[:,None,:,:] + inputs_g*(1-mask_small_TUS)[:,None,:,:]
                #choice
                out_g = classifier_gen(inputs_comb*mask_new[:,None,:,:])
                ...
                choice_mask = choice_mask.cpu().detach().numpy()
                sample_indices += (batch_idx*bs+np.nonzero(choice_mask)[0]).tolist()
                query_indices += (batch_idx*bs+np.nonzero(1-choice_mask)[0]).tolist()

            query_subset= torch.utils.data.Subset(random_test_data_subset, query_indices)
            sample_subset= torch.utils.data.Subset(random_test_data_subset, sample_indices)
            query_loader = torch.utils.data.DataLoader(query_subset, batch_size=bs, shuffle=False)
            sample_loader = torch.utils.data.DataLoader(sample_subset, batch_size=bs, shuffle=False)
            query_masks = random_testmasks[query_indices]
            sample_masks = random_testmasks[sample_indices]

            correct = 0
            total = 0
            total_cost = 0

            print("query, sample", len(query_subset), len(sample_subset))
            for batch_idx, (inputs, targets) in enumerate(sample_loader):
                inputs, targets = inputs.to(device), targets.to(device)
                mask_S = sample_masks[batch_idx*bs:(1+batch_idx)*bs]
                mask_new = copy.deepcopy(mask_S)
                mask_new[:, getT2(T_val)] = 1
                mask_small_TUS = copy.deepcopy(mask_S)
                mask_small_TUS[:, getT2(T_small)] = 1
                ...
                inputs_g = decoder(encoder(inputs*mask_small_TUS[:,None,:,:]))
                inputs_comb = inputs*mask_small_TUS[:,None,:,:] + inputs_g*(1-mask_small_TUS)[:,None,:,:]
                outputs = classifier_gen(inputs_comb*mask_new[:,None,:,:])
                _, predicted = torch.max(outputs.data, 1)
                total += targets.size(0)
                correct += predicted.eq(targets.data).cpu().sum()
                total_cost += len(T_small)*inputs.shape[0]
            print("sample acc", correct/total)

            for batch_idx, (inputs, targets) in enumerate(query_loader):
                inputs, targets = inputs.to(device), targets.to(device)
                mask_S = query_masks[batch_idx*bs:(1+batch_idx)*bs]
                mask_new = copy.deepcopy(mask_S)
                mask_new[:, getT2(T_val)] = 1
                mask_new = mask_new.reshape((-1,32,32)).to(device)
                ...
                total += targets.size(0)
                correct += predicted.eq(targets.data).cpu().sum()
                total_cost += len(T_val)*inputs.shape[0]
    ...
    with open(os.path.join(save_metrics_folder, f"results_choice_val_LSH__c_{c}__T_{args.warm_end_load_T}__warm_end_end_to_end_{args.warm_end_end_to_end}__warm_end_loss_{args.warm_end_loss}__generate_during_test_{args.generate_during_test}__generate_for_min_{args.generate_for_min}__choice_frac_{args.choice_frac}.pkl"), "wb") as f:
