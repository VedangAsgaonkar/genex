...
if args.warm_end:
    ...
    print("DATA SIZE*ACC", total*acc.item())
...
print(sum(p.numel() for p in encoder.parameters())+sum(p.numel() for p in decoder.parameters())+sum(p.numel() for p in classifier.parameters()))
input()
...
if(args.test):
    ...
    with torch.no_grad():
        ...
        correct = 0
        total = 0
        test_size = len(data_test_subset)
        local_accs = [0]
        local_total = 0
        for batch_idx, (inputs, targets) in enumerate(testloader):
            inputs, targets = inputs.to(device), targets.to(device)
            ...
            total += targets.size(0)
            correct += predicted.eq(targets.data).cpu().sum()
            local_accs[-1] += predicted.eq(targets.data).cpu().sum()
            local_total += targets.size(0)
            if local_total > test_size//10:
                local_accs[-1] = local_accs[-1]/local_total
                local_accs.append(0)
                local_total = 0
        acc = correct/total
        print(local_accs[:-1])
        print(f"Warm end : Test accuracy {acc}")
    ...
    with open(os.path.join(save_metrics_folder, f"results_LSH__c_{c}__T_{args.warm_end_load_T}__warm_end_end_to_end_{args.warm_end_end_to_end}__warm_end_loss_{args.warm_end_loss}__generate_during_test_{args.generate_during_test}__generate_for_min_{args.generate_for_min}.pkl"), "wb") as f:
