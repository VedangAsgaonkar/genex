...
from models.efficient_net import EfficientNetClassifier
from models.dcgan import DCGANDecoder
from models.wideresnet import Wide_ResNet
from models.vitb import VitBClassifier
...
if args.train_gen:
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            mask_S = set_to_mask(getT2(s), 4096) # bs*1024
            # mask_S = set_to_mask(getT2(s), 1024) # bs*1024
            mask_S = mask_S.reshape((64, 64)).to(device)
...
classifier = EfficientNetClassifier().to(device)
c = args.cluster_id
...
data_test_subset = torch.utils.data.Subset(val_dataset, val_indices[c])
testloader = torch.utils.data.DataLoader(data_test_subset, batch_size=bs, shuffle=False)
trainsets = [all_sets[i] for i in train_indices[c]]
testsets = [val_all_sets[i] for i in val_indices[c]]
trainmasks = torch.stack([set_to_mask(getT2(s), 4096) for s in trainsets])
testmasks = torch.stack([set_to_mask(getT2(s), 4096) for s in testsets])
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                for batch_idx, (inputs, targets) in enumerate(testloader):
                    ...
                    # mask_S = mask_S.reshape((-1,64,64)).to(device)
...
while epoch<epoch_max and not args.no_greedy: # WARNING: hard coded epochs
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        mask_new = mask_new.reshape((-1,64,64)).to(device) # bs*32*32

        optimizer.zero_grad()
        mask_new = mask_new.reshape((-1,64,64)) # bs*32*32
        ...
        batch_loss.backward()
        optimizer.step()
        print(f"train batch {batch_idx}/{(len(data_subset)//bs)+1}")
    with torch.no_grad():
        encoder.eval()
        decoder.eval()
        classifier.eval()
...
if args.warm_end:
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            mask_S = mask_S.reshape((-1,64,64)).to(device)
            mask_small_TUS = mask_small_TUS.reshape((-1,64,64)).to(device)
            mask_new = mask_new.reshape((-1,64,64)).to(device)
            optimizer.zero_grad()
...
if(args.test):
    ...
    classifier_gen.load_state_dict(torch.load(os.path.join(save_folder, classifier_gen_path)))
    ...
    with torch.no_grad():
        ...
        query_subset= torch.utils.data.Subset(test_data_subset, query_indices)
        sample_subset= torch.utils.data.Subset(test_data_subset, sample_indices)
        ...
        query_masks = testmasks[query_indices]
        sample_masks = testmasks[sample_indices]
        ...
        correct_arr = np.zeros(10)
        total_arr = np.zeros(10)

        bucket_size = int(math.ceil(len(sample_subset)/10))
        for batch_idx, (inputs, targets) in enumerate(sample_loader):
            ...
            correct_arr[batch_idx%bucket_size] += predicted.eq(targets.data).cpu().sum().item()
            total_arr[batch_idx%bucket_size] += targets.size(0).item()

        print("sample acc", correct/total)
        bucket_size = int(math.ceil(len(query_subset)/10))
