...
def greedy(encoder, decoder, classifier, loss, greedy_subset, sets, T, epoch_loss, device, args, testing = False, topk=1):
    ...
    new_feature = torch.zeros((batch_size, 256, 4096)) # bs*256*1024
    ...
    mask = torch.stack([set_to_mask(getT2(s), 4096) for s in sets]) # bs*1024
    ...
    new_mask = new_mask.reshape((-1,256,64,64)) # bs*256*32*32
    mask = mask.reshape((-1,256,64,64)) # bs*256*32*32
    assert new_mask.shape == (batch_size,256,64,64)
    ...
    assert new_X.shape == (batch_size,256,3,64,64)
    
    new_X = new_X.reshape((-1,3,64,64)) # 256bs*3*32*32
    new_mask = new_mask.reshape((-1, 64,64)) # 256bs*32*32
    mask = mask.reshape((-1, 64,64)) # 256bs*32*32
    ...
    epoch_loss = epoch_loss.cpu()
    gain = (epoch_loss - final_losses)*complement_mask
    m = torch.min(gain)
    gain = (epoch_loss - final_losses)*complement_mask + (1-complement_mask)*m
